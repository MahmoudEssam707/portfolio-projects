...
def choose_db_path():
    root.filename = filedialog.askopenfilename(title="Select Database File", filetypes=(("Database files", "*.db"), ("All files", "*.*")))
    conn = sqlite3.connect(root.filename)
    cursor = conn.cursor()
...
btn_show_hotel = tk.Button(button_frame, text="View Hotel Info", command=show_hotel_info)
btn_show_hotel.grid(row=0, column=0, padx=10, pady=5)

btn_show_event = tk.Button(button_frame, text="View Event Info", command=show_event_info)
...
btn_show_customer = tk.Button(button_frame, text="View Customer Info", command=show_customer_info)
btn_show_customer.grid(row=0, column=3, padx=10, pady=5)

btn_show_worker = tk.Button(button_frame, text="View Worker Info", command=show_worker_info)
btn_show_worker.grid(row=0, column=4, padx=10, pady=5)

btn_show_manager = tk.Button(button_frame, text="View Manager Info", command=show_manager_info)
